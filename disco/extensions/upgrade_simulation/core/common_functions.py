...
def run_selective_master_dss(master_filepath=None, **kwargs):
    ...
    for line in tlines:
        if (
            ("Solve".lower() in line.lower())
            or ("Export".lower() in line.lower())
            or ("Plot".lower() in line.lower())
        ):
...
def circuit_solve_and_check(raise_exception=False, **kwargs):
    ...
    if enable_pydss_solve:  # if pydss solver is also to be used
        pydss_pass_flag = pydss_solve_and_check(
...
def get_circuit_info():
    ...
    if data_dict["source_num_nodes"] > 1:
        data_dict["feeder_head_basekv"] = round(
            data_dict["feeder_head_basekv"] * math.sqrt(3), 1
        )
    data_dict["substation_xfmr"] = None

    all_xfmr_df = get_all_transformer_info(compute_loading=False)
    all_xfmr_df["substation_xfmr_flag"] = all_xfmr_df.apply(
        lambda x: int(data_dict["source_bus"].lower() in x["bus_names_only"]), axis=1
    )
    if len(all_xfmr_df.loc[all_xfmr_df["substation_xfmr_flag"] == True]) > 0:
        data_dict["substation_xfmr"] = all_xfmr_df.loc[
            all_xfmr_df["substation_xfmr_flag"] == True
        ].to_dict(orient="records")[0]
        data_dict["substation_xfmr"]["kVs"] = convert_list_string_to_list(
            data_dict["substation_xfmr"]["kVs"]
        )
        # this checks if the voltage kVs are the same for the substation transformer
        data_dict["substation_xfmr"]["is_autotransformer_flag"] = (
            len(set(data_dict["substation_xfmr"]["kVs"])) <= 1
        )
    return data_dict
...
def get_all_transformer_info(compute_loading=True, upper_limit=1.5):
    ...
    for index, row in all_df.iterrows():
        ...
        all_df.at[index, "bus_names_only"] = [
            a.split(".")[0].lower() for a in row["buses"]
        ]
        ...
        if compute_loading:
            ...
            extract_magang = dss.CktElement.CurrentsMagAng()[
                : 2 * row["phases"]
            ]  # extract elements based on num of ph
...
def get_all_line_info(compute_loading=True, upper_limit=1.5, ignore_switch=True):
    ...
    for index, row in all_df.iterrows():
        ...
        if round(kv_b1) != round(kv_b2):
            raise Exception(
                "To and from bus voltages ({} {}) do not match for line {}".format(
...
def get_regcontrol_info(correct_PT_ratio=False, nominal_voltage=None):
    ...
    all_df[
        "at_substation_xfmr_flag"
    ] = False  # by default, reg control is considered to be not at substation xfmr
    ...
    for index, row in all_df.iterrows():
        ...
        all_df.at[index, "transformer_bus1"] = dss.CktElement.BusNames()[0].split(".")[
            0
        ]
        all_df.at[index, "transformer_bus2"] = dss.CktElement.BusNames()[1].split(".")[
            0
        ]
        if sub_xfmr_present and (
            row["transformer"] == sub_xfmr_name
        ):  # if reg control is at substation xfmr
            all_df.at[index, "at_substation_xfmr_flag"] = True
    all_df = all_df.reset_index(drop=True).set_index("name")
    if correct_PT_ratio:
        if nominal_voltage is None:
            raise Exception(
                "Nominal voltage not provided to correct regcontrol PT ratio."
            )
...
def get_capacitor_info(nominal_voltage=None, correct_PT_ratio=False):
    ...
    capcontrol_df.rename(
        columns={
            "name": "capcontrol_name",
            "capacitor": "capacitor_name",
            "type": "capcontrol_type",
            "equipment_type": "capcontrol_present",
        },
        inplace=True,
    )
    ...
    if correct_PT_ratio and (len(capcontrol_df) > 0):
        if nominal_voltage is None:
            raise Exception(
                "Nominal voltage not provided to correct capacitor bank PT ratio."
            )
        all_df["old_PTratio"] = all_df["PTratio"]
...
def check_dss_run_command(command_string):
    ...
    if result != "":
        raise Exception(
            f"DSS run_command failed with message: {result}. \nCommand: {command_string}"
        )


def get_bus_voltages(
    upper_limit=1.05, lower_limit=0.95, raise_exception=True, **kwargs
):
    ...
    for bus_name in all_bus_names:
        ...
        data_dict = {"name": bus_name, "voltages": dss.Bus.puVmagAngle()[::2]}
        data_dict["Max per unit voltage"] = max(data_dict["voltages"])
        data_dict["Min per unit voltage"] = min(data_dict["voltages"])
        data_dict["Phase imbalance"] = (
            data_dict["Max per unit voltage"] - data_dict["Min per unit voltage"]
        )

        # check for overvoltage violation
        if data_dict["Max per unit voltage"] > upper_limit:
            data_dict["Overvoltage violation"] = True
            data_dict["Max voltage_deviation"] = (
                data_dict["Max per unit voltage"] - upper_limit
            )
        else:
        ...
        if data_dict["Min per unit voltage"] < lower_limit:
            data_dict["Undervoltage violation"] = True
            data_dict["Min voltage_deviation"] = (
                lower_limit - data_dict["Min per unit voltage"]
            )
        else:
    ...
    undervoltage_bus_list = list(
        all_df.loc[all_df["Undervoltage violation"] == True]["name"].unique()
    )
    overvoltage_bus_list = list(
        all_df.loc[all_df["Overvoltage violation"] == True]["name"].unique()
    )
...
def determine_available_line_upgrades(line_loading_df):
    ...
    line_upgrade_options = line_upgrade_options.loc[
        line_upgrade_options.astype(str).drop_duplicates(subset=property_list).index
    ]
    line_upgrade_options.reset_index(drop=True, inplace=True)
    line_upgrade_options = line_upgrade_options.reset_index().rename(
        columns={"index": "name"}
    )
...
def determine_available_xfmr_upgrades(xfmr_loading_df):
    ...
    xfmr_upgrade_options = xfmr_upgrade_options.loc[
        xfmr_upgrade_options.astype(str).drop_duplicates().index
    ]
    xfmr_upgrade_options.reset_index(drop=True, inplace=True)
    xfmr_upgrade_options = xfmr_upgrade_options.reset_index().rename(
        columns={"index": "name"}
    )
...
def convert_summary_dict_to_df(summary_dict=None):
    return (
        ...
        .unstack()
        .reset_index()
        .rename(columns={"level_0": "Stage", "level_1": "Parameter", 0: "Value"})
    )

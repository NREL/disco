...
def compute_transformer_costs(xfmr_upgrades_df=None, xfmr_cost_database=None, **kwargs):
    ...
    added_xfmr_df = xfmr_upgrades_df.loc[
        (xfmr_upgrades_df["Upgrade_Type"].isin(upgrade_type_list))
        & (xfmr_upgrades_df["Action"] == "add")
    ]
    computed_cost = []
    for index, row in added_xfmr_df.iterrows():
        unit_cost = xfmr_cost_database.loc[
            (xfmr_cost_database["rated_kVA"] == row["rated_kVA"])
            ...
            & (
                xfmr_cost_database["primary_connection_type"]
                == row["primary_connection_type"]
            )
            & (
                xfmr_cost_database["secondary_connection_type"]
                == row["secondary_connection_type"]
            )
            & (xfmr_cost_database["num_windings"] == row["num_windings"])
        ]["cost"]
        if (
            len(unit_cost) > 0
        ):  # if there are more than one rows, the first one is chosen in random
            ...
            closest = xfmr_cost_database.loc[
                abs(
                    xfmr_cost_database[backup_deciding_property]
                    - row[backup_deciding_property]
                ).idxmin()
            ]
            row[output_cost_field] = closest["cost"]
            row[output_count_field] = 1
            comment_string = (
                f"Transformer {row['final_equipment_name']}: Exact cost not available. "
        ...
        if (misc_database is not None) and (not misc_database.empty):
            misc_xfmr_fields = {
                "replace": "Replace transformer (fixed cost)",
            ...
            if (row["Upgrade_Type"].lower() == "upgrade") and (
                ...
                fixed_cost = misc_database.loc[
                    misc_database["Description"] == field_name
                ]
            ...
            if not fixed_cost.empty:
                row[output_cost_field] += misc_database.loc[
                    misc_database["Description"] == field_name
                ]["total_cost"].values[0]
...
def reformat_xfmr_files(xfmr_upgrades_df=None, xfmr_cost_database=None):
    ...
    """
    xfmr_upgrades_df.rename(
        columns={"kVA": "rated_kVA", "windings": "num_windings"}, inplace=True
    )
    xfmr_upgrades_df["conns"] = xfmr_upgrades_df["conns"].apply(
        lambda x: convert_list_string_to_list(x)
    )
    xfmr_upgrades_df["kVs"] = xfmr_upgrades_df["kVs"].apply(
        lambda x: convert_list_string_to_list(x)
    )
    ...
    xfmr_cost_database["secondary_kV"] = xfmr_cost_database["secondary_kV"].astype(
        float
    )
...
def compute_line_costs(line_upgrades_df=None, line_cost_database=None, **kwargs):
    ...
    properties_list = list(set(output_columns_list) - {"type"})
    upgrade_type_list = ["upgrade", "new (parallel)"]
    added_line_df = line_upgrades_df.loc[
        (line_upgrades_df["Upgrade_Type"].isin(upgrade_type_list))
        & (line_upgrades_df["Action"] == "add")
    ]
    computed_cost = []
    for index, row in added_line_df.iterrows():
        ...
        unit_cost = line_cost_database.loc[
            (line_cost_database["phases"] == row["phases"])
        ...
        if (
            ...
            # (or whatever backup deciding property is passed) (ignore other properties)
            closest = line_cost_database.loc[
                abs(
                    line_cost_database[backup_deciding_property]
                    - row[backup_deciding_property]
                ).idxmin()
            ]
            row[output_cost_field] = closest["cost_per_m"] * line_length_m
            comment_string = (
                f"Line {row['final_equipment_name']}: Exact cost not available. "
...
def reformat_line_files(line_upgrades_df=None, line_cost_database=None):
    ...
    """
    line_upgrades_df.rename(
        columns={"normamps": "ampere_rating", "kV": "voltage_kV"}, inplace=True
    )
    line_upgrades_df["ampere_rating"] = (
        line_upgrades_df["ampere_rating"].astype(float).round(2)
    )
    line_upgrades_df["phases"] = line_upgrades_df["phases"].astype(int)
    line_upgrades_df["voltage_kV"] = (
        line_upgrades_df["voltage_kV"].astype(float).round(2)
    )
    # assign original equipment length to new equipment
    line_upgrades_df["length"] = line_upgrades_df.groupby("original_equipment_name")[
        "length"
    ].transform("first")

    line_cost_database["ampere_rating"] = (
        line_cost_database["ampere_rating"].astype(float).round(2)
    )
    line_cost_database["phases"] = line_cost_database["phases"].astype(int)
    line_cost_database["voltage_kV"] = (
        line_cost_database["voltage_kV"].astype(float).round(2)
    )
...
def compute_capcontrol_cost(voltage_upgrades_df=None, controls_cost_database=None):
    ...
    capcontrol_fields = {
        "add_new_cap_controller": "New capacitor controller added",
        "change_cap_control": "Capacitor controller settings modified",
    }
    cost_database_fields = {
        "add_new_cap_controller": "Add new capacitor controller",
        "change_cap_control": "Change capacitor controller settings",
        "replace_cap_controller": "Replace capacitor controller",
    }
    empty_cap_cost_dict = {
        "type": output_rows,
        "count": [0] * len(output_rows),
        "total_cost_usd": [0] * len(output_rows),
    }
    ...
    count_new_controller = cap_upgrades_df.loc[
        capcontrol_fields["add_new_cap_controller"]
    ].sum()
    unit_cost_new_controller = controls_cost_database.loc[
        controls_cost_database["Type"] == cost_database_fields["add_new_cap_controller"]
    ]["cost"].values[0]
    total_cost_new_controller = count_new_controller * unit_cost_new_controller

    count_setting_changes = cap_upgrades_df.loc[
        capcontrol_fields["change_cap_control"]
    ].sum()
    unit_cost_setting_changes = controls_cost_database.loc[
        controls_cost_database["Type"] == cost_database_fields["change_cap_control"]
    ]["cost"].values[0]
...
def compute_voltage_regcontrol_cost(
    voltage_upgrades_df=None, controls_cost_database=None, keyword="RegControl"
):
    ...
    output_columns_list = ["type", output_count_field, output_cost_field, "comment"]
    upgrade_fields = {
        "add_new_reg_control": "New voltage regulator control",
        "change_reg_control": "Voltage regulator control settings modified",
        "add_new_transformer": "Volt",
        "add_substation_ltc": "Volt",
        "change_ltc_control": "Volt",
    }
    cost_database_fields = {
        "add_new_reg_control": "Add new voltage regulator controller",
        "change_reg_control": "Change voltage regulator controller settings",
        "replace_reg_control": "Replace voltage regulator controller",
    }

    output_rows = [
        "New in-line voltage regulator",
        "In-line voltage regulator control setting change",
    ...
    for field in computation_fields:
        ...
        unit_cost = controls_cost_database.loc[
            controls_cost_database["Type"] == cost_database_fields[field]
        ]["cost"].values[0]

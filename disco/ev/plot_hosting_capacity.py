...
def plot_capacity_V(file, caseA, caseB, caseC, caseD, feeder):    
    ...
    vplot2 = ax1.plot(range(len(file)), file[caseB], c='r', marker = '+', label = "Volt violation MW [range:(0.95, 1.05)]") 
    vplot3 = ax1.plot(range(len(file)), file[caseC], c='g', marker = '*', label = "Volt violation MW [range:(0.975, 1.05)]") 
    vplot4 = ax1.plot(range(len(file)), file[caseD], c='b', marker = '.', label = "Volt violation MW [range:(0.985, 1.05)]") 
    ax1.margins(x=0)
    ...
    ax1.set_xlabel('Feeder Node (where highest is furthest from substation)', fontsize=33) ##'Load indices, according to distance from SS \n(load #' + str(len(file)) + ' is the furthest from SS)'
    # plt.tight_layout()
...
def plot_capacity_thermal_2(file, caseA, caseB, caseC, feeder):    
    ...
    thplot2 = ax1.plot(range(len(file)), file[caseB], c='r', marker = '+', label = "Thermal violation MW[100% loading]")
    thplot3 = ax1.plot(range(len(file)), file[caseC], c='b', marker = '*', label = "Thermal violation MW[120% loading]")
    ax1.margins(x=0)
    ...
    ax1.grid()
    #ax1.set_xlabel('Load # according to distance from SS', fontsize=40)
    ax1.set_xlabel('Feeder Node (where highest is furthest from substation)', fontsize=30) #ax1.set_xlabel('Load indices, according to distance from SS \n(load #' + str(len(file)) + ' is the furthest from SS)', fontsize=40)
    plt.tight_layout()
...
def plot_capacity_thermal_1(file, caseA, caseB, feeder, extra):    
    fig, ax1= plt.subplots(nrows=1,figsize=(14, 8))
    thplot1 = ax1.plot(range(len(file)), file[caseA], c='k', marker = 'o', label = "Initial load")  
    thplot2 = ax1.plot(range(len(file)), file[caseB], c='r', marker = '+', label = "Thermal violation MW")
    ax1.margins(x=0)
    ax1.tick_params(axis='both', which='major', labelsize=33)
    ax1.tick_params(axis='both', which='minor', labelsize=33)
    ax1.set_ylabel('Power (MW)', fontsize=33)
    ...
    ax1.grid()
    #ax1.set_xlabel('Load # according to distance from SS', fontsize=40)
    ax1.set_xlabel('Feeder Node (where highest is furthest from substation)', fontsize=33) #ax1.set_xlabel('Load indices, according to distance from SS \n(load #' + str(len(file)) + ' is the furthest from SS)', fontsize=40)
    # fig.tight_layout()
    plt.savefig(str(feeder) + '_Cap_by_thermal_limit_' + str(extra) + '.png', bbox_inches = 'tight', dpi=150)
...
if __name__ == "__main__": 
    ...
    for i in range(len(lmt)):
        v_output_df.append(pd.read_csv("Hosting_capacity_test_" + str(feeder_folder) + "_" + str(lmt[i]) + ".csv"))
    ...
    node_distance["Distance"] = Bus_Distance    #node_distance.to_csv("    ") save as csv if required
    ...
    plot_capacity_V(plot_df, 'Initial_MW', 'Volt_Violation_0.95', 'Volt_Violation_0.975', 'Volt_Violation_0.985', feeder_folder)

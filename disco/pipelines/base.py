...
class PipelineTemplate:
    ...
    def get_command_options(self, section, params_type):
        ...
        for param, value in params.items():
            ...
            if value is True:
                options += f" {dash_param}"
            else:
                options += f" {dash_param} {value}"
        return options

    def get_transform_options(self, section):
        return self.get_command_options(section, TemplateParams.TRANSFORM_PARAMS)
...
class PipelineCreatorBase(ABC):
    ...
    def get_prescreen_auto_config_script(self):
        return os.path.join(".", "pipeline-prescreen-auto-config.sh")
    
    def get_simulation_auto_config_script(self):
        return os.path.join(".", "pipeline-simulation-auto-config.sh")

    def get_postprocess_auto_config_script(self):
        return os.path.join(".", "pipeline-postprocess-auto-config.sh")
    
    def get_postprocess_command_script(self):
        return os.path.join(".", "pipeline-postprocess-command.sh")
    ...
    def create_prescreen_auto_config_script(self):
        ...
        auto_config_command = " && \\\n".join([transform_command, config_command, prescreen_command])
        
        text = "#!/bin/sh\n\n" + auto_config_command
        script_file = self.get_prescreen_auto_config_script()
        create_script(script_file, text)
        return script_file
    
    def create_simulation_auto_config_script(self):
        ...
        if self.template.contains_prescreen():
            ...
            transform_command = self.make_model_transform_command()
            config_command = self.make_disco_config_command(TemplateSection.SIMULATION)
            auto_config_command = " && \\\n".join([transform_command, config_command])
        
        text = "#!/bin/sh\n\n" + auto_config_command
        script_file = self.get_simulation_auto_config_script()
        create_script(script_file, text)
        return script_file

    def create_postprocess_auto_config_script(self):
        """Create script for generating postprocess config file"""
        command_file = self.create_postprocess_command_file()
        ...
        text = "#!/bin/sh\n\n" + auto_config_command
        script_file = self.get_postprocess_auto_config_script()
        create_script(script_file, text)
        return script_file
    
    def create_postprocess_command_script(self):
        """Create script for running postprocess command"""
        command = self.make_postprocess_command()
        text = "#!/bin/sh\n\n" + command
        script_file = self.get_postprocess_command_script()
        create_script(script_file, text)
        return script_file
    
    def create_postprocess_command_file(self):
        text_file = "pipeline-postprocess-command.txt"
        script_file = self.create_postprocess_command_script()
    ...
    def make_prescreen_stage(self):
        self.stage_num += 1
        auto_config_command = self.create_prescreen_auto_config_script()
    ...
    def make_simulation_stage(self):
        self.stage_num += 1
        auto_config_command = self.create_simulation_auto_config_script()
    ...
    def make_postprocess_stage(self):
        self.stage_num += 1
        auto_config_command = self.create_postprocess_auto_config_script()

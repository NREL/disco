...
def aggregate_deployments(job_outputs_path):
    ...
    config_file = os.path.join(os.path.dirname(job_outputs_path), "config.json")
    config = PyDssConfiguration.deserialize(config_file)
    all_summaries_dict = dict()
    
    
        
    base_case = [job for job in config.iter_jobs() if isinstance(job, DeploymentParameters) and job.model.is_base_case][0]
    print(base_case)
    for job in config.iter_jobs():
        project_path = os.path.join(
            job_outputs_path, job.name
        )
